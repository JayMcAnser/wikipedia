...
class MergeEngine {
  constructor(template = undefined, options = {}) {
    ...
    this.blockReplace =  {
        bold: {openTag: '<span class="mk-font-bold mk-font-hdr">', closeTag: '</span>'},
        italic: {openTag: '<span class="mk-italic">', closeTag: '</span>'}
    }
    if (options.blockReplace) {
      for (const key in options.blockReplace) {
        if (this.blockReplace[key]) {
          if (options.blockReplace[key].openTag) {
            this.blockReplace[key].openTag = options.blockReplace[key].openTag
          }
          if (options.blockReplace[key].closeTag) {
            this.blockReplace[key].closeTag = options.blockReplace[key].closeTag
          }
        } else {
          this.blockReplace[key] = options.blockReplace[key]
        }
      }
    }
    let vm = this;
