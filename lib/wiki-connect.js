...
class WikiConnect {
  ...
  async artistByQId(qId, name) {
    ...
    bio.images = this.getImages(result.data.entities[qId])
    return bio;
  }
  ...
  _encodeTitle(fileName) {
    let title = fileName.replace(/^(image|file?)\:/i, '')
    //titlecase it
    title = title.charAt(0).toUpperCase() + title.substring(1)
    //spaces to underscores
    title = escape(title.trim().replace(/ /g, '_'))
    return title
  }
  /**
  ...
  _urlEncode(fileName) {
    ...
    // escape does not encode ć to %C4%87 but to %u0107
